...
def build_second_layer_text(level_key):
    texts = {
        "low": (
            "Во втором слое видно, что часть вас всё ещё умеет сохранять спокойствие, даже если в отдельных ситуациях тревога усиливается."
        ),
        "medium": (
            "Во втором слое видно привычное внутреннее волнение, которое становится заметнее в ситуациях ожидания, неопределённости или перегруза."
        ),
        "high": (
            "Во втором слое видно, что напряжение у вас может быстро нарастать и переходить из обычного волнения в устойчивую тревожную реакцию."
        ),
        "very_high": (
            "Во втором слое видно, что за тревогой может стоять уже не только напряжение, но и накопленная внутренняя перегрузка или истощение."
        ),
    }
    return texts[level_key]
...
def build_profile_payload(answer_values):
    ...
    return {
        ...
        "score": total_score,
        "level": main_level,
        "primary_result": main_level,
        "secondary_result": second_layer_key,
