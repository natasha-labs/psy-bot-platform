    ...
    with open(image, "rb") as photo:
        await context.bot.send_photo(
            ...
            caption=(
                ...
                "Не спеши.\n"
                "Просто побудь с ним несколько секунд."
            ),
...
async def send_contact_step(update, context):
    await context.bot.send_message(
        ...
        text=(
            ...
            "Не нужно называть это правильно.\n"
            "Можно просто заметить.\n\n"
            "Если ничего не происходит — это тоже нормально."
        ),
        reply_markup=next_from_contact_keyboard(),
    )
...
async def send_life_step(update, context):
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=(
            "Если хочется, посмотри —\n"
            "есть ли в твоей жизни что-то с похожим ощущением.\n\n"
            "Можно задать себе вопрос к этому образу.\n"
            "Или ничего не задавать.\n\n"
